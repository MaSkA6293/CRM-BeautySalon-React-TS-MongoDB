...
const FormEditRecord = ({
    ...
    const [startRecordTime, setStartRecordTime] = useState(currentRecord.timeStart);
    const [finishRecordTime, setfinishRecordTime] = useState(currentRecord.timeEnd);

    const handlerChangeTimeStart = (e: React.SyntheticEvent<HTMLInputElement>) => {
    ...
    const handlerDeletRecord = (e: React.SyntheticEvent<HTMLButtonElement>) => {
        e.preventDefault();
        const quest = window.confirm("Вы действительно хотите удалить встречу");
        if (quest) {
                ...
                <form className="addNewWrite">
                    ...
                    <div className="addNewWrite__wraper">
                        <label>
                            {" "}
                            Выберите дату
                            <input type="date" value={formatDateToInput(date)} onChange={handleChangeDate}></input>
                        </label>
                        <label>
                            Выберите клиента
                        ...
                        <label>
                            ...
                            <input type="time" value={startRecordTime} onChange={handlerChangeTimeStart} />
                        </label>
                        <label>
                            Время окончания встречи
                            <input type="time" value={finishRecordTime} onChange={handlerChangeTimeEnd} />
                        </label>
                        ...
                        <button className="button addNewWrite__button save" onClick={handlerEditRecord}>
                            Сохранить изменения
                        </button>
                        <button className={"button addNewWrite__button delet"} onClick={handlerDeletRecord}>
                            Удалить встречу
                        </button>
                    </div>
                </form>
