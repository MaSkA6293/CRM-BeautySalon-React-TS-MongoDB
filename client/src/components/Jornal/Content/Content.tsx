...
const Content = () => {
    ...
    const { date, workTime, clients, records, colors } = useSelector(({ stateApp, clients, records, colors }: any) => {
        return {
            date: stateApp.date,
            workTime: stateApp.workTime,
            clients: clients,
            records: records.filter((record: IRecord) => record.date === formatDateToSave(stateApp.date)),
            colors: colors,
        };
    });
    ...
    const openModalAddRecord = (e: React.SyntheticEvent<HTMLDivElement>): void => {
    ...
    const handlerRecordClick = (e: React.SyntheticEvent<HTMLDivElement>): void => {
        const recordId = e.currentTarget.dataset.recordid;
        const currentRecord = records.find((r: IRecord) => r.id === Number(recordId));
        if (currentRecord) {
    ...
    return (
        <div className="Content">
                      ...
                      return (
                          <div
                              ...
                              style={{
                                  ...
                                  top:
                                      diffDates(+new Date(`${r.date}  ${r.timeStart}`), +new Date(`${r.date}`)) -
                                      9 * 60 +
                                      "px",
            ...
            {workTime.map((time: string, index: number) => {
                return <ContentItem key={index} time={time} addNewWrite={openModalAddRecord} />;
            })}
