...
import { editClient, deletClient, getClientsRequest } from "../../actions/actionClients";
...
const Clients = () => {
    ...
    return (
        <>
            ...
            {addIsOpen ? <ModalAddClient modalIsOpen={addIsOpen} closeModal={() => setAddIsOpen(false)} /> : ""}
            {editIsOpen ? (
                <FormEditClient
                    modalIsOpen={editIsOpen}
                    closeModal={() => setEditIsOpen(false)}
                    currentClient={selectedClient}
                    editClient={(data: IClient, callback: () => void) => dispatch(editClient(data, callback))}
                    deletClient={(_id: number, callback: () => void) => dispatch(deletClient(_id, callback))}
                />
            ) : (
                ""
            )}
